...
#test_dir=data/dev_beamformit_dereverb_stats_max_seg
test_dir=data/segmented_dev_chime3
#test_dir=data/callhome_5folds/1/test
exp_dir=experiment/pretrained
#exp_dir=$exp_dir_all/1 
output_dir=$exp_dir/result/$modelname
test_output_dir=$output_dir/callhome_test
echo "Fold 1 Modelname is $modelname"

# Forward the model to get region proposals, confidence score and speaker embeddings 
if [ $stage -le 0 ]; then
  bash -x scripts/eval_cpu.sh --cmd "$train_cmd --mem 10G" --nj 10 \
          --nclass 5963 \
          $test_dir $exp_dir $modelname callhome_test
fi

# Cluster the speaker embeddings and apply NMS
if [ $stage -le 1 ]; then
  $MYPYTHON scripts/cluster_nms.py $test_output_dir/detections.pkl $test_output_dir/rttm_num_spk \
          --num_cluster $test_dir/reco2num_spk --nms_thres $nms_thres --thres $thres --cluster_type $cluster_type \
          --merge-chime6-segments 1
fi
...
#if [ $stage -le 3 ]; then
#  result_dir_all=$exp_dir/results
#  mkdir -p $result_dir_all || exit 1;
#
#  cat $exp_dir/result/$modelname/callhome_test/rttm_num_spk \
